...
- Publisher canvas ownership: `--owned-pin` compares raw bytes, but Obsidian re-serializes a `.canvas` on open (tabs, compact nodes), so any canvas Douglas views becomes un-updatable. Record a canonical-JSON hash (parsed, sorted keys, nodes and edges sorted by id, ignoring Obsidian-added `fromSide`/`toSide` defaults) in the pin and compare that instead. <!-- agent: instagram-saved-catalog/session -->
