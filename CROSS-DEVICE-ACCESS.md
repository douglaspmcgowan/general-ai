...
- Engineer-brief Git coverage is partial. A read-only audit on 2026-07-31 found that of 13 current Engineer briefs, five have no copy on a published remote: `START HERE - Cross-Device Harness.md` had no copy anywhere outside the vault, `Codex Harness Slowness Diagnosis.md` and `Second Brain Cloud Access Decision Brief.md` exist only under the gitignored `general-claude\.local\` tree, `Receiving Device Receipt Schema.md` is now published in `research/`, and `Obsidian Headless Scoping Run.md` has a partial repository copy that is smaller than the vault original. Those briefs still reach a phone through Obsidian Sync; they do not reach GitHub or a cloud agent. The earlier claim that all Engineer documents had Git-backed copies held only when unpushed branches and the gitignored tree were counted.
- The full inventory of what exists only on this computer is deliberately kept off this public repository, because it enumerates client and business repositories together with their branch and working-tree state. It lives in the active vault at `Claude/Engineer/Local-Only Context Audit.md` and in Docket under the stable card `agent-harness--local-only-context-audit-2026-07-31`. In summary it records 28 Git roots, seven local branches whose commits exist on no remote, three worktrees holding unique unpublished work, twelve repositories carrying their project contract as untracked files, and one 4.8 GB folder with no Git repository and no inventory entry.
