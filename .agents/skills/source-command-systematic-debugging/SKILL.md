---
name: "source-command-systematic-debugging"
description: "Use when encountering any bug, test failure, or unexpected behavior, before proposing fixes"
---

# source-command-systematic-debugging

Use this skill when the user asks to run the migrated source command `systematic-debugging`.
...
4. **Gather Evidence in Multi-Component Systems**
   ...
   **Example (multi-layer system):**
   ```text
   Layer 1 — workflow: record whether the required credential variable exists, without reading its value.
   Layer 2 — build process: record whether the child process receives that variable.
   Layer 3 — signing provider: list available credential identities through the platform's safe metadata command.
   Layer 4 — signer: run the platform signer in verbose mode against a disposable fixture.
   ```
...
1. **Create Failing Test Case**
   ...
   - Use the `source-command-test-driven-development` skill for writing proper failing tests
...
- **Root-cause tracing** — trace the failing value backward through callers to its original trigger.
- **Defense in depth** — after identifying the source, add proportionate validation at the boundaries the bad state crossed.
- **Condition-based waiting** — replace arbitrary timeouts with polling for the actual required state.

**Related skills:**
- **`source-command-test-driven-development`** — create the failing regression test in Phase 4.
- **`source-command-verification-before-completion`** — verify the assembled fix before claiming success.
